...
app1.controller('ctrl2', function($scope) {
    ...
    $scope.hypotenuse = 'Side A = ' +  $scope.first + ' and Side B = ' + $scope.second + ' Hypotenuse = '
	+ Math.sqrt(($scope.first * $scope.first) + ($scope.second * $scope.second));
	};
});

app1.controller('svsuCourses', function($scope){
	...
	$scope.random = $scope.courses[Math.floor((Math.random() * 10))];
